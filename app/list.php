...
<div class="row">
  <div class="col-md-12">
    ...
    if (!$config['showSwitch'] && !is_who_login('admin')) : ?>
      ...
      if ($config['cnb_status']) :
        ...
        if ($cnbDate < date('Y-m-d', strtotime("- $listDate day"))) {
          $cnbDate = date('Y-m-d');
          echo '
          <script>
      ...
      <div class="btn-toolbar">
        <div class="btn-group">
          <?php if ($config['cnb_status']) : ?>
            <a class="btn btn-danger btn-mini" href="?date=<?php echo $cnbDate; ?>">当前<?php echo $cnbCount; ?></a>
            <a class="btn btn-primary btn-mini" href="list.php">今日<?php echo $cnbTodayCount; ?></a>
            <a class="btn btn-mini" href="?date=<?php echo date('Y-m-d', strtotime("-1 day")); ?>">昨日<?php echo $cnbYesterdayCount; ?></a>
            <?php
        ...
        <?php if (!$config['cnb_status'] && is_who_login('admin')) : ?>
          <div class="btn-group">
    ...
    <div class="col-md-2 col-xs-5">
      <form action="<?php echo $_SERVER['SCRIPT_NAME']; ?>" method="get">
        <div class="input-group">
          ...
          <input type="text" class="form-control form-date input-sm" name="date" value="<?php echo $config['cnb_status'] ? date('Y/m/d/') : date('Y/m/d/'); ?>" readonly="readonly">
  ...
  <script>
      ...
      bootbox.confirm({
        ...
        callback: function(result) {
          if (result == true) {
            ...
            $.post("del.php", { 'recycle_url_array': check_val });
            new $.zui.Messager("放入回收站成功", { type: "success", icon: "ok-sign" }).show();
            window.setTimeout(function() { window.location.reload(); }, 1500)
          } else {
            new $.zui.Messager("取消回收", { type: "primary", icon: "info-sign" }).show();
          }
        }
      });
    }
      ...
      bootbox.confirm({
        ...
        callback: function(result) {
          if (result == true) {
            ...
            $.post("del.php", { 'del_url_array': check_val },
              function(data) {
                if (data.search('success') > 0) {
                  new $.zui.Messager("删除成功", { type: "success", icon: "ok-sign" }).show();
                } else {
                  new $.zui.Messager("删除失败 请登录后再删除!", { type: "danger", icon: "exclamation-sign" }).show();
                }
                window.setTimeout(function() { window.location.reload(); }, 1500)
              });
          } else {
            new $.zui.Messager("取消删除", { type: "primary", icon: "info-sign" }).show();
          }
        }
      });
    }
      ...
      if (null != objNameList) {
        for (var i = 0; i < objNameList.length; i++) {
          if (objNameList[i].checked == true) {
            if (type != 'checkall') objNameList[i].checked = false;
          } else {
            if (type != 'uncheckall') objNameList[i].checked = true;
          }
        }
      }
    }
    ...
    clipboard.on('success', function(e) {
      new $.zui.Messager("复制直链成功", { type: "success", icon: "ok-sign" }).show();
    });
    clipboard.on('error', function(e) {
      new $.zui.Messager("复制失败", { type: "danger", icon: "exclamation-sign" }).show();
    });

    //懒加载
    var lazy = new Lazy({
      onload: function(elem) { console.log(elem) },
      delay: 300,
    })
    ...
    // 更改网页标题
    <?php if ($config['cnb_status']) : ?>
      document.title = "图床广场 - 今日上传<?php echo $cnbTodayCount; ?>张 昨日<?php echo $cnbYesterdayCount; ?>张 - <?php echo $config['title']; ?>"
    <?php else : ?>
