    ...
    if ($.zui.store.length() > 1) {
        ...
        $.zui.store.forEach(function(key, value) { // 遍历所有本地存储的条目
            ...
            if (value['url'] !== undefined) {
                ...
                let videoExts = ['mp4','webm','ogg','mov','avi','mkv','flv','wmv','m4v','3gp','ts'];
                let urlExt = value['url'].split('.').pop().toLowerCase().split('?')[0];
                let isVideo = value['fileType'] === 'video' || videoExts.indexOf(urlExt) !== -1;
                let mediaHtml = '';
                if (isVideo) {
                    mediaHtml = '<div class="video-thumb" style="width:100%;height:258px;display:flex;align-items:center;justify-content:center;background:#000;border-radius:5px 5px 0 0;overflow:hidden;"><video src="' + value['url'] + '" style="width:100%;height:100%;object-fit:contain;" controls preload="metadata"></video></div>';
                } else {
                    mediaHtml = '<li><img src="../public/images/loading.svg" data-image="' + value['thumb'] + '" data-original="" alt="CNB图床"></li>';
                }
                $('.listNum').append('<div class="col-md-4 col-sm-6 col-lg-3"><div class="card">' + mediaHtml + '<div class="bottom-bar"><a href="' + value['url'] + '" target="_blank"><i class="icon icon-picture" data-toggle="tooltip" title="打开" style="margin-left:10px;"></i></a><a href="#" class="copy" data-clipboard-text="' + value['url'] + '" data-toggle="tooltip" title="复制链接" style="margin-left:10px;"><i class="icon icon-copy"></i></a><a href="info.php?history=' + v_url.path + '" data-toggle="tooltip" title="详细信息" target="_blank" style="margin-left:10px;"><i class="icon icon-info-sign"></i></a><a href="down.php?history=' + v_url.path + '" data-toggle="tooltip" title="下载文件" target="_blank" style="margin-left:10px;"><i class="icon icon-cloud-download"></i></a><a href="#" data-toggle="tooltip" title="删除记录" class="Remove"id="' + value['srcName'] + '" style="margin-left:10px;"><i class="icon icon-remove-sign"></i></a></a><a href="' + value['del'] + '" target="_blank"><i class="icon icon-trash" data-toggle="tooltip" title="删除文件" style="margin-left:10px;"></i></a><a href="#" data-toggle="tooltip" title="源文件名" class="copy text-ellipsis" data-clipboard-text="' + value['srcName'] + '" style="margin-left:10px;">' + value['srcName'] + '</a></div></div></div>')
            }
        })
